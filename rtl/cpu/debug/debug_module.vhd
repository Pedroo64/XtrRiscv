        ...
        if rising_edge(clk_i) then
            if enable_i = '1' then
                case current_st is
                    when st_transfer =>
                        instr_valid_o <= '1';
                        instr_data_o(6 downto 0) <= RV32I_OP_SYS;
                        instr_data_o(31 downto 20) <= CSR_DM_DATA0;
                        if dm_regs_command_control_alias.write = '1' then
                            instr_data_o(19 downto 15) <= (others => '0');
                            instr_data_o(14 downto 12) <= RV32I_FN3_CSRRC;
                            instr_data_o(11 downto 7) <= dm_regs_command_control_alias.regno(4 downto 0);
                        else
                            instr_data_o(19 downto 15) <= dm_regs_command_control_alias.regno(4 downto 0);
                            instr_data_o(14 downto 12) <= RV32I_FN3_CSRRW;
                            instr_data_o(11 downto 7) <= (others => '0');
                        end if;
                    when st_progbuf0 =>
                        instr_valid_o <= '1';
                        instr_data_o <= dm_regs.progbuf0;
                    when st_progbuf1 =>
                        instr_valid_o <= '1';
                        instr_data_o <= dm_regs.progbuf1;
                    when st_progbuf2 =>
                        instr_valid_o <= '1';
                        instr_data_o <= dm_regs.progbuf2;
                    when st_progbuf3 =>
                        instr_valid_o <= '1';
                        instr_data_o <= dm_regs.progbuf3;
                    when st_ebreak =>
                        instr_valid_o <= '1';
                        instr_data_o(6 downto 0) <= RV32I_OP_SYS;
                        instr_data_o(11 downto 7) <= (others => '0');
                        instr_data_o(14 downto 12) <= (others => '0');
                        instr_data_o(19 downto 15) <= (others => '0');
                        instr_data_o(31 downto 20) <= x"001";
                    when st_dret =>
                        instr_valid_o <= '1';
                        instr_data_o <= x"7b200073"; -- DRET
                    when others =>
                        instr_valid_o <= '0';
                        instr_data_o <= (others => 'X');
                end case;
            end if;
        end if;
    end process;
